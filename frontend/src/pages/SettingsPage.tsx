...
export function SettingsPage() {
  ...
  const getMonitorCode = () => {
    return `javascript:void(function(){if(window._mm){clearInterval(window._mm);window._mm=null;document.title=document.title.replace(/\\[MuMarket\\][^]*/g,'').trim();alert('PARADO');return}if(!window.location.hostname.includes('mudream.online')){alert('ERRO: Abra mudream.online/pt/market primeiro!');return}var T='${token}';var A='${API_URL}';var all=[],total=0,errCount=0;function qp(o){return fetch('/api/graphql',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/graphql-response+json'},body:JSON.stringify({operationName:'GET_ALL_LOTS',query:'query GET_ALL_LOTS($offset:NonNegativeInt,$limit:NonNegativeInt,$sort:LotsSortInput){lots(limit:$limit,offset:$offset,sort:$sort){Lots{id source type gearScore Prices{value Currency{code title}} Currencies{code title}} Pagination{total}}}',variables:{filter:{},limit:200,offset:o,sort:{field:'LOT_FIELD_UPDATED_AT',type:'SORT_TYPE_DESC'}}}),credentials:'same-origin'}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);var ct=r.headers.get('content-type')||'';if(ct.indexOf('json')===-1)throw new Error('Resposta nao e JSON. Faca login no MuDream.');return r.json()})}function p(){all=[];total=0;qp(0).then(function(d){errCount=0;if(!d.data||!d.data.lots||!d.data.lots.Lots)throw new Error('Dados invalidos');var lots=d.data.lots.Lots;total=d.data.lots.Pagination?d.data.lots.Pagination.total:lots.length;lots.forEach(function(l){all.push({id:l.id,name:l.source||'?',type:l.type||'',gearScore:l.gearScore||0,Prices:l.Prices||[],options:(l.Currencies||[]).map(function(c){return(c.code||'').toUpperCase()})})});var pages=Math.min(Math.ceil(total/200),5);var prom=[];for(var i=1;i<pages;i++)prom.push(qp(i*200));return Promise.all(prom)}).then(function(res){res.forEach(function(d){if(d&&d.data&&d.data.lots&&d.data.lots.Lots){d.data.lots.Lots.forEach(function(l){all.push({id:l.id,name:l.source||'?',type:l.type||'',gearScore:l.gearScore||0,Prices:l.Prices||[],options:(l.Currencies||[]).map(function(c){return(c.code||'').toUpperCase()})})})}});document.title='[MuMarket] '+all.length+' de '+total+' itens';return fetch(A+'/api/monitoring/bookmarklet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:T,items:all})})}).then(function(r){if(r)return r.json()}).then(function(d){if(d&&d.error){document.title='[MuMarket] Erro: '+d.error}else if(d&&d.matches>0){document.title='[MuMarket] '+d.matches+' MATCHES!';if('Notification' in window&&Notification.permission==='granted')new Notification('MuMarket!',{body:d.matches+' itens!'});else if('Notification' in window&&Notification.permission!=='denied')Notification.requestPermission()}}).catch(function(e){errCount++;console.error('[MuMarket]',e.message);if(errCount>3)document.title='[MuMarket] ERRO: '+e.message})}p();window._mm=setInterval(p,10000);alert('Monitor INICIADO! Mantenha esta aba aberta.')})()`;
  };
